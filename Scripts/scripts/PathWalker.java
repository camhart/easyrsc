...
public class PathWalker extends Script
    ...
    public static class Path {
        ...
        public Node[] getReversePath() {
        	Node[] ret = new Node[n.length];
        	for(int c = 0; c < n.length; c++) {
        		ret[c] = n[n.length - (c + 1)];
        	}
        	return ret;
        }
    }
...
//        }
    ...
    public final Location getNearestBank(int x, int y) {
        int best_dist = Integer.MAX_VALUE;
        Location best_loc = null;
        for (Location loc : locations) {
            if (!loc.bank) continue;
            // Could always calculate a path to check the
            // distance more accurately, but that is much slower.
            int dist = distanceTo(x, y, loc.x, loc.y);
            if (dist < best_dist) {
                best_dist = dist;
                best_loc = loc;
            }
    ...
    private Node[] astar(Node start, Node goal) {
        ...
        ArrayDeque<Node> open = new ArrayDeque<Node>(32);
        open.add(start);
        start.open = true;
    ...
    private static Node getLowestFScore(ArrayDeque<Node> open) {
        Node best_n = null;
